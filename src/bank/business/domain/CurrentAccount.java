...
public class CurrentAccount implements Credentials {
	...
	public Transfer transfer(OperationLocation location,
		...
		if((location instanceof ATM && amount >= 5000) == false) {
	...
	public void completeTransfer(Transfer selectedTransfer) {
		try {
			selectedTransfer.getDestinationAccount().depositAmount(selectedTransfer.getAmount());
		} catch (BusinessException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		selectedTransfer.endTransfer();
		selectedTransfer.getDestinationAccount().transfers.add(selectedTransfer);
		
	}
	
	public void stopTransfer(Transfer selectedTransfer) {
		// TODO Auto-generated method stub
		try {
			this.depositAmount(selectedTransfer.getAmount());
		} catch (BusinessException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		selectedTransfer.cancelTransfer();
	}
